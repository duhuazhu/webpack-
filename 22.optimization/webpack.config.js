...
module.exports={
    ...
    optimization: {
        splitChunks: {
            chunks: "all",
            minSize:30 * 1024, //分割chunk最小单位,
            maxSize: 0,  //最大没有限制
            minChunks: 1, //提取的chunk最少引入一次
            maxAsyncRequests: 5,//按需加载的时候加载文件最大数量
            maxInitialRequests: 3,//入口js文件最大并行请求数量
            automaticNameDelimiter: "~",//名称连接符
            name:true,//可以使用命名规则
            cacheGroups: {
                //分割chunk的组
                //node_modules 文件会被打包到vendors 组的chunk中
                vendors:{
                    test:/[\\/]node_modules[\\/]/,
                    //打包的优先级
                    priority: -10,
                ...
                default:{
                    ...
                    minChunks: 2,
                    priority: -20,
                    //如果当前要打包的模块,和之前要打包的模块是同一个 就会复用  不是重新打包模块
                    reuseExistingChunk: true
                }
            }
        },
        //将当前模块记录的hash 单独打包成一个文件  runtime文件
        //解决缓存失效
        runtimeChunk: {
